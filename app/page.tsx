...
6つの占術データから導かれた5枚のカードについて、クライアントが「なぜ私のことがわかるんですか？」と思わず涙ぐむような、深い洞察と具体性のあるリーディングメッセージを書いてください。
...
【執筆ルール — 最重要】
1. 各メッセージは250〜350文字。短すぎるのは絶対にNG
2. 以下の3段構成で書く:
   - 第1段: カードが「なぜあなたに現れたか」を占術データの具体的な要素と結びつけて説明する（例:「ライフパス7を持つあなたは、本質的に真理を探究する魂。だからこそ隠者のカードが…」）
   - 第2段: そこから読み取れる、その人の「あるある」な具体的行動パターンや感覚を描写する（例:「周囲が盛り上がっている場でも、ふと一人になりたくなる瞬間がありませんか？」）
   - 第3段: 今のその人に向けた具体的なメッセージや行動のヒント
3. 占術の専門用語は1〜2個使ってよいが、必ず自然な文脈で噛み砕く
4. 相談内容があれば、5枚すべてのメッセージが有機的にその悩みに寄り添う
5. 「〜ではないでしょうか」「〜という感覚はありませんか？」など、当たっていると思わせる問いかけを必ず入れる
6. 占い本にありがちな抽象的・汎用的な表現は避け、その人のデータだからこそ言えることを書く
7. **必ず純粋なJSON形式で出力**（Markdownのバッククォートは不要）
          ...
          {allRevealed && (
            <div className="tarot-summary">
              <div className="tarot-summary-inner">
                ...
                <h2 className="tarot-summary-title">5枚のカードが語るもの</h2>
                <p className="tarot-summary-text">
                  あなたの本質から導きまで、6つの占術が選んだ5枚のカードが揃いました。<br />
                  それぞれのカードは独立したメッセージではなく、<br />
                  ひとつの大きな物語としてあなたの今を映し出しています。
                </p>
