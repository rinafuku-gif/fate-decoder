...
function formatWesternForPrompt(w: WesternData): string {
  ...
  const balance = `エレメントバランス（太陽〜土星の7天体）: 火${eb.fire} 地${eb.earth} 風${eb.air} 水${eb.water}（${w.dominantElement}が優勢）`

  const aspects = w.tightAspects.map(a => a.desc).join(', ')

  const lines = [
    `  天体配置: ${mainPlanets}`,
    moonNote ? `  月の星座注記: ${moonNote}` : '',
    w.retrograding.length > 0 ? `  逆行中: ${w.retrograding.join('・')}` : '',
    `  ${w.moonPhaseEmoji}月相: ${w.moonPhase}`,
    `  ${balance}`,
    aspects ? `  主要アスペクト（天体同士の角度関係）: ${aspects}` : '',
  ].filter(Boolean)
...
export default function FateDecoder() {
  ...
  // モーダル表示時に背景スクロールを抑制
  useEffect(() => {
    document.body.style.overflow = detailModal ? 'hidden' : ''
  ...
  const handleTarotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const safeName    = sanitizeForAI(formData.name, 50)
      const safeConcern = sanitizeForAI(formData.concern || '', 500)
...
【対象者】
${safeName} (${formData.year}年${formData.month}月${formData.day}日生まれ)
...
`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const safeName       = sanitizeForAI(formData.name, 50)
      const safeBirthPlace = sanitizeForAI(formData.birthPlace || '', 100)
      const safeConcern    = sanitizeForAI(formData.concern || '', 500)
...
【対象者】
名前: ${safeName} (${formData.year}年${formData.month}月${formData.day}日生まれ / ${formData.bloodType}型 / ${safeBirthPlace || '未入力'}出身)
...
【出力フォーマット — 必ずprologue/chapters/finalの3つを返すこと】
{
  "prologue": {
    "tag": "#はじめに",
  ...
  "chapters": [
    {
      "tag": "#占術名 #キーワード",
      "title": "第1章：（テーマ）",
      "text": "データを使った性格分析。800文字以上。アクション提案はここに書かない。"
    }
  ],
  "final": {
    "tag": "#まとめ",
    ...
    "magic": "運気を変える具体的なアクション1〜2文（例：赤い靴下を履く）。ここにだけ書く"
  }
}
`
          ...
          <div className="input-card">
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="ニックネームでもOK" required maxLength={50} />
              </div>
              <div className="form-group">
              ...
              {readingMode === 'short' ? (
                ...
                <div className="form-group">
                  ...
                  <textarea value={formData.concern} onChange={(e) => setFormData({ ...formData, concern: e.target.value })} rows={4} placeholder="例：今の仕事を続けるべきか迷っています..." maxLength={500} />
                </div>
              )}
          ...
          <div className="input-card">
            ...
            <form onSubmit={handleCompatSubmit}>
              ...
              <div className="compat-person-section">
                ...
                <div className="form-group">
                  <label>お名前 <span className="required">*</span></label>
                  <input type="text" value={person2.name} onChange={(e) => setPerson2({ ...person2, name: e.target.value })} placeholder="ニックネームでもOK" required maxLength={50} />
                </div>
                <div className="form-group">
                  <label>生年月日 <span className="required">*</span></label>
                  <div className="row-3">
